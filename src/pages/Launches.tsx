  ...
  return (
    <section ref={ref} style={{ backgroundColor: '#1C1A17', minHeight: '50vh' }} className="flex items-center justify-center py-24">
      <div className="launches-hero text-center px-6 max-w-[900px]">
        <p className="mono-label mb-4" style={{ color: '#1C1A17' }}>NEW INDUSTRIAL / COMMERCIAL LAUNCHES</p>
        ...
        <p className="font-sans text-lg mx-auto" style={{ color: 'rgba(255,255,255,0.8)', maxWidth: 700 }}>
...
function GatePlusHero() { return (<section id="gateplus" className="relative w-full flex items-end overflow-hidden scroll-mt-20" style={{ height: '75vh', minHeight: 500, backgroundColor: '#1C1A17' }}>
  ...
  <div className="relative z-10 w-full max-w-[1128px] mx-auto px-6 pb-16">
    <span className="inline-block font-sans text-xs font-semibold px-4 py-1.5 rounded-full mb-4" style={{ backgroundColor: '#4A3E3D', color: '#FFFFFF' }}>NEW LAUNCH — B2 Industrial</span>
    ...
    <p className="font-sans text-xl mb-2" style={{ color: 'rgba(255,255,255,0.9)' }}>@ Tukang, Jurong</p>
    <p className="font-sans text-base mb-6 max-w-[600px]" style={{ color: 'rgba(255,255,255,0.8)' }}>
...
function GatePlusOverview() {
  ...
  return (<section ref={ref} id="gateplus-details" style={{ backgroundColor: '#F7F5F0' }} className="py-20"><div className="max-w-[1128px] mx-auto px-6"><div className="gplus-overview grid grid-cols-1 lg:grid-cols-2 gap-12 items-start"><div>
    <p className="mono-label mb-3" style={{ color: '#1C1A17' }}>PROJECT OVERVIEW</p>
    ...
    {[{ label: 'Developer', value: 'Tukang Project Pte. Ltd.' }, { label: 'Project Type', value: 'Industrial B2' }, { label: 'Site Area', value: '~18,687 sqm / 201,145 sqft' }, { label: 'Max GFA', value: '~46,717.50 sqm / 502,863 sqft' }, { label: 'Tenure', value: '33 years from 27 Aug 2025' }, { label: 'Est. TOP', value: '30 January 2029' }, { label: 'Est. Legal Completion', value: '30 January 2032' }, { label: 'Sustainability', value: 'Green Mark 2021 Platinum SLE + Maintainability Badge' }, { label: 'Architect', value: '3P Ecotecture Pte Ltd' }, { label: 'M&E Engineer', value: 'Rankine & Hill (Singapore) Pte Ltd' }, { label: 'C&S Engineer', value: 'BCK & Partners Pte Ltd' }].map((item, i) => (
      <div key={i} className="flex justify-between items-center py-3 px-4 rounded-lg" style={{ backgroundColor: i % 2 === 0 ? '#fff' : 'transparent' }}>
        <span className="font-sans text-sm" style={{ color: '#1C1A17' }}>{item.label}</span><span className="font-sans text-sm font-medium text-right" style={{ color: '#1C1A17', maxWidth: '60%' }}>{item.value}</span>
      </div>
    ))}
  </div></div></div></section>)
}
...
function GatePlusUnits() {
  ...
  const unitTypes = [
    { name: 'Grand Suite (L1)', count: '24 units', levels: 'Level 1', height: '7,000mm / 6,300mm', sizes: ['2,228 sqft (AG5)', '2,217 sqft (AG5a)', '2,174 sqft (AG5b)', '2,217 sqft (AG5c)', '2,282 sqft (AGc2)', '2,293 sqft (AG)', '2,347 sqft (AG4)', '2,411 sqft (AG2/AG3)', '2,486 sqft (AG1)', '2,971 sqft (AG6)'], desc: 'Ground floor premium units with highest ceiling clearance, ideal for heavy machinery and direct container loading.', color: '#1C1A17' },
    { name: 'Standard Suite (L2-L8)', count: '190 units', levels: 'Levels 2 to 8', height: '6,125mm', sizes: ['1,615 sqft (A/Ac)', '1,625 sqft (A1/A2/A3)', '1,658 sqft (Ac1)', '1,711 sqft (Ac2/Ac3)', '1,722 sqft (Ac4)', '1,765 sqft (A4)', '1,668 sqft (A5)', '2,820 sqft (D)'], desc: 'Versatile mid-level production units with ramp-up access for 20-foot containers. Perfect for manufacturing and light assembly.', color: '#1C1A17' },
    { name: 'Pent Suite (L9-L10)', count: '51 units', levels: 'Levels 9 & 10', height: '6,125mm', sizes: ['1,615 sqft (D1/D2)', '1,636 sqft (Dc2)', '1,658 sqft (Dc1/Dc3)', '1,701 sqft (D3/D4)', '2,056 sqft (D5)', '2,530 sqft (Dc)', '14,596 sqft (HVP)'], desc: 'Top-floor units with panoramic views, sky terraces, and premium finishes. Exclusive roof garden and function room access.', color: '#1C1A17' },
  ]
  return (<section ref={ref} className="py-24" style={{ backgroundColor: '#fff' }}><div className="max-w-[1128px] mx-auto px-6"><div className="text-center mb-12"><p className="mono-label mb-3">265 PRODUCTION UNITS + 2 CANTEENS</p><h3 className="section-heading">Unit Types & Configurations</h3></div><div className="space-y-8">{unitTypes.map((u, i) => (<div key={i} className="gplus-unit-card rounded-xl p-8" style={{ border: '1px solid #E8E4DC' }}><div className="grid grid-cols-1 lg:grid-cols-3 gap-8"><div><div className="w-3 h-3 rounded-full mb-3" style={{ backgroundColor: u.color }} /><h4 className="font-serif text-2xl font-medium mb-1" style={{ color: '#1C1A17' }}>{u.name}</h4><p className="font-mono text-xs mb-4" style={{ color: '#1C1A17' }}>{u.count} | {u.levels}</p><div className="flex items-center gap-2 mb-2"><ArrowUp size={14} color={u.color} /><span className="font-sans text-sm" style={{ color: '#1C1A17' }}>Ceiling: {u.height}</span></div><p className="font-sans text-sm leading-relaxed" style={{ color: '#1C1A17' }}>{u.desc}</p></div><div className="lg:col-span-2"><p className="font-sans text-xs font-semibold uppercase mb-3" style={{ color: '#1C1A17' }}>Available Sizes</p><div className="flex flex-wrap gap-2">{u.sizes.map((s, j) => (<span key={j} className="font-sans text-sm px-3 py-2 rounded-lg" style={{ backgroundColor: `${u.color}15`, color: '#1C1A17' }}>{s}</span>))}</div></div></div></div>))}</div><div className="mt-8 p-6 rounded-xl" style={{ backgroundColor: '#F7F5F0', border: '1px solid #E8E4DC' }}><h4 className="font-serif text-xl font-medium mb-4" style={{ color: '#1C1A17' }}>Industrial Canteens</h4><div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{[{ name: 'Canteen 1 (Unit 06)', size: '4,327 sqft', power: '300A TPN' }, { name: 'Canteen 2 (Unit 26)', size: '3,197 sqft', power: '250A TPN' }].map((c, i) => (<div key={i} className="bg-white rounded-lg p-4" style={{ border: '1px solid #E8E4DC' }}><p className="font-sans text-sm font-medium" style={{ color: '#1C1A17' }}>{c.name}</p><p className="font-sans text-xs mt-1" style={{ color: '#1C1A17' }}>{c.size} | {c.power}</p></div>))}</div></div></div></section>)
}
...
function CTGoldHero() { return (<section id="ctgold" className="relative w-full flex items-end overflow-hidden scroll-mt-20" style={{ height: '75vh', minHeight: 500, backgroundColor: '#1C1A17' }}>
  ...
  <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(23,23,28,0.95) 0%, rgba(23,23,28,0.5) 40%, transparent 70%)' }} />
  <div className="relative z-10 w-full max-w-[1128px] mx-auto px-6 pb-16">
    <span className="inline-block font-sans text-xs font-semibold px-4 py-1.5 rounded-full mb-4" style={{ backgroundColor: '#C45D8C', color: '#FFFFFF' }}>100% FULLY SOLD — B1/B2 Mixed-Use</span>
    ...
    <p className="font-sans text-xl mb-2" style={{ color: 'rgba(255,255,255,0.9)' }}>Singapore</p>
    <p className="font-sans text-base mb-6 max-w-[600px]" style={{ color: 'rgba(255,255,255,0.8)' }}>
...
function CTGoldOverview() {
  ...
  return (<section ref={ref} id="ctgold-details" style={{ backgroundColor: '#F7F5F0' }} className="py-20"><div className="max-w-[1128px] mx-auto px-6"><div className="ctgold-overview grid grid-cols-1 lg:grid-cols-2 gap-12 items-start"><div>
    ...
    {[{ label: 'Project Name', value: 'CT Gold' }, { label: 'Project Type', value: 'B1 / B2 Mixed-Use Industrial & Commercial' }, { label: 'Building Height', value: '10+ Storeys (estimated)' }, { label: 'Facade', value: 'Glass Curtain Wall with Gold Accent Framework' }, { label: 'Ground Floor', value: 'Integrated F&B, Retail & Showroom Units' }, { label: 'Parking', value: 'Helical Ramp Parking Structure' }, { label: 'Signature Feature', value: 'Iconic Gold Framework Design' }, { label: 'Sustainability', value: 'Landscaped Greenery & Natural Ventilation' }, { label: 'Status', value: '100% Fully Sold' }].map((item, i) => (
      <div key={i} className="flex justify-between items-center py-3 px-4 rounded-lg" style={{ backgroundColor: i % 2 === 0 ? '#fff' : 'transparent' }}><span className="font-sans text-sm" style={{ color: '#1C1A17' }}>{item.label}</span><span className="font-sans text-sm font-medium text-right" style={{ color: '#1C1A17', maxWidth: '55%' }}>{item.value}</span></div>
    ))}
  </div></div></div></section>)
}
...
function CTGoldFeatures() {
  ...
  const features = [
    { icon: Building2, title: 'Contemporary Glass Architecture', body: 'Full-height glass curtain wall facade maximises natural daylight for production floors while creating a striking modern silhouette. The reflective glass reduces heat gain and enhances energy efficiency.', color: '#1C1A17' },
    { icon: Star, title: 'Signature Gold Framework', body: 'The distinctive gold-coloured structural framework creates a premium branded identity visible from major roadways. This unique architectural language sets CT Gold apart from conventional industrial developments.', color: '#1C1A17' },
    { icon: Store, title: 'Ground Floor F&B & Retail', body: "Activated street frontage with Cafe Gold and other commercial units brings life to the development. These spaces serve tenant employees and attract foot traffic from the surrounding industrial precinct.", color: '#1C1A17' },
    { icon: ParkingCircle, title: 'Helical Parking Ramp', body: 'The innovative cylindrical parking structure provides seamless vehicle circulation from ground to upper levels. This space-efficient solution eliminates traditional ramp constraints and adds an iconic architectural element.', color: '#1C1A17' },
    { icon: TreePine, title: 'Landscaped Environment', body: 'Generous landscaping surrounds the building with mature trees, green buffers, and pedestrian-friendly walkways. The design prioritises a pleasant working environment uncommon in industrial districts.', color: '#1C1A17' },
    { icon: Zap, title: 'Future-Ready Infrastructure', body: 'Designed with modern industrial needs in mind — high-capacity power provisions, smart building systems, EV charging readiness, and flexible floor plates that adapt to diverse tenant requirements.', color: '#1C1A17' },
  ]
  return (<section ref={ref} className="py-24" style={{ backgroundColor: '#fff' }}><div className="max-w-[1128px] mx-auto px-6"><div className="text-center mb-12"><p className="mono-label mb-3">DESIGN HIGHLIGHTS</p><h3 className="section-heading">Designed to Stand Out</h3><p className="font-sans text-base mt-4 mx-auto" style={{ color: '#1C1A17', maxWidth: 600 }}>CT Gold reimagines what an industrial development can look like — premium materials, thoughtful urban design, and tenant-first thinking.</p></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{features.map((f, i) => (<div key={i} className="ctgold-feature rounded-xl p-6" style={{ border: '1px solid #E8E4DC' }}><div className="w-10 h-10 rounded-lg flex items-center justify-center mb-4" style={{ backgroundColor: `${f.color}15` }}><f.icon size={20} color={f.color} /></div><h4 className="font-serif text-lg font-medium mb-3" style={{ color: '#1C1A17' }}>{f.title}</h4><p className="font-sans text-sm leading-relaxed" style={{ color: '#1C1A17' }}>{f.body}</p></div>))}</div></div></section>)
}
...
function CTGoldSpecs() {
  ...
  return (<section ref={ref} className="py-24" style={{ backgroundColor: '#fff' }}><div className="max-w-[1128px] mx-auto px-6"><div className="text-center mb-12"><p className="mono-label mb-3">SPECIFICATIONS</p><h3 className="section-heading">Project Details</h3><p className="font-sans text-sm mt-3" style={{ color: '#1C1A17' }}>Full technical specifications to be confirmed. Dimension measurements subject to final survey.</p></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{specs.map((s, i) => (<div key={i} className="ctgold-spec rounded-xl p-6" style={{ border: '1px solid #E8E4DC' }}><div className="w-10 h-10 rounded-lg flex items-center justify-center mb-4" style={{ backgroundColor: 'rgba(198,93,140,0.12)' }}><s.icon size={20} color="#C45D8C" /></div><h4 className="font-serif text-lg font-medium mb-3" style={{ color: '#1C1A17' }}>{s.title}</h4><ul className="space-y-2">{s.items.map((item, j) => (<li key={j} className="flex items-start gap-2"><CheckCircle size={14} color="#C45D8C" className="mt-0.5 flex-shrink-0" /><span className="font-sans text-sm" style={{ color: '#1C1A17' }}>{item}</span></li>))}</ul></div>))}</div></div></section>)
}

function CTGoldDisclaimer() { return (<section className="py-12" style={{ backgroundColor: '#F7F5F0' }}><div className="max-w-[1128px] mx-auto px-6"><div className="rounded-xl p-6 flex items-start gap-4" style={{ backgroundColor: '#fff', border: '1px solid #E8E4DC' }}><Info size={20} color="#4A3E3D" className="flex-shrink-0 mt-0.5" /><div><p className="font-sans text-sm font-medium mb-1" style={{ color: '#1C1A17' }}>Important Notice</p><p className="font-sans text-sm leading-relaxed" style={{ color: '#1C1A17' }}>All information, dimensions, and specifications for CT Gold are based on preliminary architectural impressions and are subject to final survey and developer confirmation. Unit sizes, pricing, tenure details, and exact specifications will be announced closer to launch.</p></div></div></div></section>) }
...
function GenerationsHero() { return (<section id="generations" className="relative w-full flex items-end overflow-hidden scroll-mt-20" style={{ height: '75vh', minHeight: 500, backgroundColor: '#1C1A17' }}>
  ...
  <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(23,23,28,0.95) 0%, rgba(23,23,28,0.5) 40%, transparent 70%)' }} />
  <div className="relative z-10 w-full max-w-[1128px] mx-auto px-6 pb-16">
    <span className="inline-block font-sans text-xs font-semibold px-4 py-1.5 rounded-full mb-4" style={{ backgroundColor: '#78B0B5', color: '#FFFFFF' }}>NEW LAUNCH — B1 Industrial</span>
    ...
    <p className="font-sans text-xl mb-2" style={{ color: 'rgba(255,255,255,0.9)' }}>71 Tannery Lane, Singapore 347807</p>
    <p className="font-sans text-base mb-6 max-w-[600px]" style={{ color: 'rgba(255,255,255,0.8)' }}>
...
function GenerationsOverview() {
  ...
  return (<section ref={ref} id="generations-details" style={{ backgroundColor: '#F7F5F0' }} className="py-20"><div className="max-w-[1128px] mx-auto px-6"><div className="gen-overview grid grid-cols-1 lg:grid-cols-2 gap-12 items-start"><div>
    ...
    {[{ label: 'Project Name', value: 'Generations@Tannery' }, { label: 'Developer', value: 'Providence Estates (Tannery) Pte Ltd' }, { label: 'Address', value: '71 Tannery Lane, Singapore 347807' }, { label: 'District', value: 'District 13' }, { label: 'Project Type', value: 'Industrial B1' }, { label: 'Tenure', value: 'Freehold (Estate in Fee Simple)' }, { label: 'Site Area', value: '3,150.10 SQM' }, { label: 'Storeys', value: '12-Storey Industrial Development' }, { label: 'Total Units', value: '54 Production Units + 5 Industrial Canteens' }, { label: 'Parking', value: '51 Lots (10 EV Charging Lots)' }, { label: 'Lifts', value: '6 High-Capacity KONE Lifts' }, { label: 'MRT', value: '6-min walk to Mattar MRT (DT25)' }, { label: 'Vacant Possession', value: '31 December 2030' }, { label: 'Legal Completion', value: '31 December 2033' }].map((item, i) => (
      <div key={i} className="flex justify-between items-center py-3 px-4 rounded-lg" style={{ backgroundColor: i % 2 === 0 ? '#fff' : 'transparent' }}><span className="font-sans text-sm" style={{ color: '#1C1A17' }}>{item.label}</span><span className="font-sans text-sm font-medium text-right" style={{ color: '#1C1A17', maxWidth: '60%' }}>{item.value}</span></div>
    ))}
  </div></div></div></section>)
}
...
function GenerationsUnits() {
  ...
  const unitTypes = [
    { name: 'Industrial Canteens (Level 1)', count: '5 units', levels: 'Ground Floor', height: '6.125M FTF', desc: 'Ground floor industrial canteens in various sizes, curated to support diverse dining concepts. From intimate cafés to contemporary eateries, these spaces foster a lively communal atmosphere where occupants and the wider community can gather, recharge and connect.', color: '#1C1A17' },
    ...
    { name: 'Production Units (Level 5)', count: '14 dual-key feasible', levels: 'Level 5', height: '4.375M FTF', desc: 'Mid-floor production units with dual-key feasibility — dividing a single unit into two fully independent businesses, each with its own entrance, identity, and room to grow. Opening up twice the opportunity within a single investment.', color: '#1C1A17' },
    { name: 'Production Units (Levels 6-9)', count: 'TBC', levels: 'Levels 6 to 9', height: '4.200M FTF', desc: 'Upper-floor production units with 4.200M floor-to-floor height. Connected to 6 high-capacity KONE lifts including service lifts. Suitable for software development, call centres, co-working spaces, e-commerce fulfilment, media production, and data centres.', color: '#1C1A17' },
    { name: 'Production Units (Levels 10-12)', count: '3 private lift units', levels: 'Levels 10 to 12', height: '4.100M - 4.725M FTF', desc: 'Top-floor premium production units with the highest ceilings (4.725M on Level 10). Three exclusive units on Levels 6-8 come with private lift access for added prestige and privacy. Ideal for businesses seeking a distinguished address.', color: '#1C1A17' },
  ]
  return (<section ref={ref} className="py-24" style={{ backgroundColor: '#fff' }}><div className="max-w-[1128px] mx-auto px-6"><div className="text-center mb-12"><p className="mono-label mb-3">54 PRODUCTION UNITS + 5 INDUSTRIAL CANTEENS</p><h3 className="section-heading">Unit Types & Configurations</h3><p className="font-sans text-base mt-4 mx-auto" style={{ color: '#1C1A17', maxWidth: 600 }}>Freehold units with flexible floor-to-floor heights and robust electrical infrastructure for diverse industrial needs.</p></div><div className="space-y-6">{unitTypes.map((u, i) => (<div key={i} className="gen-unit-card rounded-xl p-8" style={{ border: '1px solid #E8E4DC' }}><div className="grid grid-cols-1 lg:grid-cols-3 gap-8"><div><div className="w-3 h-3 rounded-full mb-3" style={{ backgroundColor: u.color }} /><h4 className="font-serif text-2xl font-medium mb-1" style={{ color: '#1C1A17' }}>{u.name}</h4><p className="font-mono text-xs mb-3" style={{ color: '#1C1A17' }}>{u.count} | {u.levels}</p><div className="flex items-center gap-2 mb-2"><ArrowUp size={14} color={u.color} /><span className="font-sans text-sm" style={{ color: '#1C1A17' }}>Ceiling: {u.height}</span></div><p className="font-sans text-sm leading-relaxed" style={{ color: '#1C1A17' }}>{u.desc}</p></div><div className="lg:col-span-2"><div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{[
    ...
    ].map((spec, j) => (<div key={j} className="bg-white rounded-lg p-3" style={{ border: '1px solid #E8E4DC' }}><p className="font-sans text-xs" style={{ color: '#1C1A17' }}>{spec.label}</p><p className="font-sans text-sm font-medium mt-1" style={{ color: '#1C1A17' }}>{spec.value}</p></div>))}</div></div></div></div>))}</div></div></section>)
}
...
function GenerationsLocation() {
  ...
  return (<section ref={ref} className="py-24" style={{ backgroundColor: '#fff' }}><div className="max-w-[1128px] mx-auto px-6"><div className="gen-loc grid grid-cols-1 lg:grid-cols-2 gap-12"><div>
    ...
    <p className="font-sans text-base leading-relaxed mb-8" style={{ color: '#1C1A17' }}>Generations@Tannery sits at <strong>71 Tannery Lane, Singapore 347807</strong> on the city's fringe in District 13. It is a quick 6-minute walk from Mattar MRT on the Downtown Line (DTL) — perfect for users who prefer to go car-lite. Stay effortlessly connected to everything that matters via the PIE and CTE.</p>
    <div className="space-y-6">
      <div className="flex items-start gap-4"><div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: 'rgba(120,176,181,0.15)' }}><Train size={18} color="#78B0B5" /></div><div><h4 className="font-sans text-sm font-semibold mb-1" style={{ color: '#1C1A17' }}>MRT Connectivity</h4><p className="font-sans text-sm" style={{ color: '#1C1A17' }}>Mattar MRT (DT25): 6-min walk</p><p className="font-sans text-sm" style={{ color: '#1C1A17' }}>MacPherson Interchange (CC10/DT26): nearby</p></div></div>
      <div className="flex items-start gap-4"><div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: 'rgba(120,176,181,0.15)' }}><MapPin size={18} color="#78B0B5" /></div><div><h4 className="font-sans text-sm font-semibold mb-1" style={{ color: '#1C1A17' }}>Key Distances</h4><p className="font-sans text-sm" style={{ color: '#1C1A17' }}>Orchard Road: 11 mins | CBD: 13 mins</p><p className="font-sans text-sm" style={{ color: '#1C1A17' }}>Changi Airport: 24 mins | Woodlands Checkpoint: 30 mins</p></div></div>
    </div>
  </div><div>
    <p className="mono-label mb-3" style={{ color: '#1C1A17' }}>PARKING & ACCESS</p>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {['51 Car Lots (10 EV Charging)', '5-Storey Ramp-up Access', '18 Premium Units with Direct Parking', '3 Private Lift Access Units', '14 Dual-Key Feasible Units', '6 High-Capacity KONE Lifts', '4-Metre Wide Lift Landing', '1 Attached Toilet Per Unit', 'Flexible Unit Layouts'].map((a, i) => (
        <div key={i} className="flex items-center gap-3 bg-white rounded-lg p-4" style={{ border: '1px solid #E8E4DC' }}><ParkingCircle size={16} color="#78B0B5" /><span className="font-sans text-sm" style={{ color: '#1C1A17' }}>{a}</span></div>
      ))}
    </div>
  </div></div></div></section>)
}
...
function GXCHero() { return (<section id="gxc" className="relative w-full flex items-end overflow-hidden scroll-mt-20" style={{ height: '75vh', minHeight: 500, backgroundColor: '#1C1A17' }}>
  ...
  <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(23,23,28,0.95) 0%, rgba(23,23,28,0.5) 40%, transparent 70%)' }} />
  <div className="relative z-10 w-full max-w-[1128px] mx-auto px-6 pb-16">
    <span className="inline-block font-sans text-xs font-semibold px-4 py-1.5 rounded-full mb-4" style={{ backgroundColor: '#5DC489', color: '#FFFFFF' }}>NEW LAUNCH — B2 Food Hub</span>
    ...
    <p className="font-sans text-xl mb-2" style={{ color: 'rgba(255,255,255,0.9)' }}>@ 1 Kallang Way</p>
    <p className="font-sans text-base mb-6 max-w-[600px]" style={{ color: 'rgba(255,255,255,0.8)' }}>
...
function GXCUnitTypes() {
  ...
  return (<section ref={ref} className="py-24" style={{ backgroundColor: '#fff' }}><div className="max-w-[1128px] mx-auto px-6"><div className="text-center mb-12"><p className="mono-label mb-3">264 MODERN + 8 HERITAGE UNITS</p><h3 className="section-heading">Unit Types for Every Food Business</h3><p className="font-sans text-base mt-4 mx-auto" style={{ color: '#1C1A17', maxWidth: 600 }}>From compact cloud kitchens to expansive deluxe production floors with mezzanine offices — GXC accommodates every scale of food operation.</p></div><div className="space-y-8">{units.map((u, i) => (<div key={i} className="gxc-unit-card rounded-xl p-8" style={{ border: '1px solid #E8E4DC' }}><div className="grid grid-cols-1 lg:grid-cols-3 gap-8"><div><div className="w-3 h-3 rounded-full mb-3" style={{ backgroundColor: u.color }} /><h4 className="font-serif text-2xl font-medium mb-1" style={{ color: '#1C1A17' }}>{u.name}</h4><p className="font-mono text-xs mb-3" style={{ color: '#1C1A17' }}>{u.tag}</p><div className="flex items-center gap-2 mb-2"><Ruler size={14} color={u.color} /><span className="font-sans text-sm" style={{ color: '#1C1A17' }}>{u.size}</span></div><div className="flex items-center gap-2 mb-2"><ArrowUp size={14} color={u.color} /><span className="font-sans text-sm" style={{ color: '#1C1A17' }}>Height: {u.height}</span></div></div><div className="lg:col-span-2"><p className="font-sans text-sm leading-relaxed mb-4" style={{ color: '#1C1A17' }}>{u.desc}</p><div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{[
...
function GXCFeatures() {
  ...
  const features = [
    { icon: Waves, title: 'Kallang Riverfront', body: 'Unique riverfront setting along the Kallang River with a dedicated promenade. Tenants and visitors enjoy waterfront views, breezy outdoor spaces, and direct access to nature — a rare amenity for an industrial development.', color: '#1C1A17' },
    { icon: TreePine, title: 'Sky Garden & Rooftop Gardens', body: 'Lush sky garden at Level 3 and rooftop gardens across the building provide green respite spaces for workers. These biophilic design elements reduce stress, improve air quality, and create a sense of well-being.', color: '#1C1A17' },
    { icon: Utensils, title: 'Food-First Infrastructure', body: 'Purpose-built for food operations with dedicated kitchen exhaust shafts, sunken floors for drainage, segregated raw/finished goods entry in deluxe units, and heavy-duty power supply. Every detail is designed with food safety and operational efficiency in mind.', color: '#1C1A17' },
    { icon: Building2, title: 'Heritage Adaptive Reuse', body: 'The 8 heritage terraced units retain distinctive architectural character while being fully modernised with contemporary infrastructure. This creative preservation adds cultural value and creates unique dining destinations.', color: '#1C1A17' },
    { icon: Zap, title: 'Green & Smart Building', body: 'Green Mark Platinum SLE with solar panels, water-efficient fittings (PUB WELS 3-tick), LED lighting (>60% energy savings), and SGBP 2-tick certified materials. Smart building systems optimise energy use and operational costs.', color: '#1C1A17' },
    { icon: Truck, title: 'Superior Logistics Design', body: '16m wide driveway, ramp-up access throughout, rooftop heavy vehicle parking across all 9 storeys, dedicated 40ft unloading bays on L2-L3, and 24ft unloading bays in every production unit. Logistics efficiency is built into every level.', color: '#1C1A17' },
  ]
...
function GXCBusinessFit() {
  ...
  const categories = [
    { title: 'Food Manufacturing', items: ['Central kitchens', 'Food manufacturing', 'Bakery & confectionery', 'Beverage bottling', 'Ready-meal production'], color: '#1C1A17' },
    { title: 'Food Service & Retail', items: ['Cloud kitchens', 'Food kiosks', 'Restaurant frontage', 'Cafes & terraces', 'Catering operations'], color: '#1C1A17' },
    { title: 'Distribution & Innovation', items: ['Cold chain logistics', 'Food distribution', 'Retail + B2B hybrid', 'Food innovation R&D', 'E-commerce fulfilment'], color: '#1C1A17' },
  ]
  return (<section ref={ref} style={{ backgroundColor: '#F7F5F0' }} className="py-24"><div className="max-w-[1128px] mx-auto px-6"><div className="text-center mb-12"><p className="mono-label mb-3">BUSINESS SUITABILITY</p><h3 className="section-heading">Perfect For Your Food Business</h3><p className="font-sans text-base mt-4 mx-auto" style={{ color: '#1C1A17', maxWidth: 600 }}>Whether you're a startup cloud kitchen or an established food manufacturer, GXC has a space designed for your operations.</p></div><div className="grid grid-cols-1 md:grid-cols-3 gap-6">{categories.map((c, i) => (<div key={i} className="gxc-fit-card rounded-xl p-6" style={{ border: '1px solid #E8E4DC', backgroundColor: '#fff' }}><div className="w-10 h-10 rounded-lg flex items-center justify-center mb-4" style={{ backgroundColor: `${c.color}15` }}><ForkKnife size={20} color={c.color} /></div><h4 className="font-serif text-xl font-medium mb-4" style={{ color: '#1C1A17' }}>{c.title}</h4><ul className="space-y-3">{c.items.map((item, j) => (<li key={j} className="flex items-center gap-2"><CheckCircle size={14} color={c.color} className="flex-shrink-0" /><span className="font-sans text-sm" style={{ color: '#1C1A17' }}>{item}</span></li>))}</ul></div>))}</div></div></section>)
}
...
function GXCConnectivity() {
  ...
  return (<section ref={ref} className="py-24" style={{ backgroundColor: '#fff' }}><div className="max-w-[1128px] mx-auto px-6"><div className="gxc-conn grid grid-cols-1 lg:grid-cols-2 gap-12"><div>
    <p className="mono-label mb-3" style={{ color: '#1C1A17' }}>CONNECTIVITY</p>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {[{ name: 'Tai Seng', time: '6 min' }, { name: 'MacPherson', time: '5 min' }, { name: 'Kaki Bukit', time: '11 min' }, { name: 'Ubi', time: '7 min' }, { name: 'Bedok', time: '17 min' }, { name: 'Paya Lebar', time: '10 min' }, { name: 'Jalan Besar', time: '8 min' }, { name: 'Lavender', time: '6 min' }].map((hub, i) => (
        <div key={i} className="flex items-center justify-between bg-white rounded-lg p-4" style={{ border: '1px solid #E8E4DC' }}><span className="font-sans text-sm" style={{ color: '#1C1A17' }}>{hub.name}</span><span className="font-sans text-xs font-medium px-2 py-1 rounded" style={{ backgroundColor: 'rgba(93,196,137,0.12)', color: '#1C1A17' }}>{hub.time}</span></div>
      ))}
    </div>
  </div></div></div></section>)
}
...
function Space18Hero() { return (<section id="space18" className="relative w-full flex items-end overflow-hidden scroll-mt-20" style={{ height: '75vh', minHeight: 500, backgroundColor: '#1C1A17' }}>
  ...
  <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(23,23,28,0.95) 0%, rgba(23,23,28,0.5) 40%, transparent 70%)' }} />
  <div className="relative z-10 w-full max-w-[1128px] mx-auto px-6 pb-16">
    <span className="inline-block font-sans text-xs font-semibold px-4 py-1.5 rounded-full mb-4" style={{ backgroundColor: '#8E69B6', color: '#FFFFFF' }}>NEW LAUNCH — B1 Freehold</span>
    ...
    <p className="font-sans text-xl mb-2" style={{ color: 'rgba(255,255,255,0.9)' }}>18 Lorong Ampas, Singapore 328779</p>
    <p className="font-sans text-base mb-6 max-w-[600px]" style={{ color: 'rgba(255,255,255,0.8)' }}>
...
function Space18Location() {
  ...
  return (<section ref={ref} className="py-24" style={{ backgroundColor: '#fff' }}><div className="max-w-[1128px] mx-auto px-6"><div className="s18-loc grid grid-cols-1 lg:grid-cols-2 gap-12"><div>
    ...
    <p className="font-sans text-base leading-relaxed mb-8" style={{ color: '#1C1A17' }}>SPACE@18 is strategically positioned in the established Balestier/Whampoa industrial precinct, offering excellent connectivity to major expressways and MRT stations. The location provides easy access to the CBD, Orchard, and other key business districts.</p>
    ...
    <p className="mono-label mb-3" style={{ color: '#1C1A17' }}>KEY FEATURES</p>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {['Full-Height Glass Wall Facade', 'Sheltered Carpark', 'Multi-Unit Merging', '7.0m Ceiling (Level 1)', '6.3m Ceiling (Levels 2-6)', '3 Loading Bays', '60A / 3-Phase Power', '1 Canteen (200A)'].map((a, i) => (
        <div key={i} className="flex items-center gap-3 bg-white rounded-lg p-4" style={{ border: '1px solid #E8E4DC' }}><CheckCircle size={16} color="#8E69B6" /><span className="font-sans text-sm" style={{ color: '#1C1A17' }}>{a}</span></div>
      ))}
    </div>
  </div></div></div></section>)
}
...
function SpaceNovaHero() { return (<section id="spacenova" className="relative w-full flex items-end overflow-hidden scroll-mt-20" style={{ height: '75vh', minHeight: 500, backgroundColor: '#1C1A17' }}>
  ...
  <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(23,23,28,0.95) 0%, rgba(23,23,28,0.5) 40%, transparent 70%)' }} />
  <div className="relative z-10 w-full max-w-[1128px] mx-auto px-6 pb-16">
    <span className="inline-block font-sans text-xs font-semibold px-4 py-1.5 rounded-full mb-4" style={{ backgroundColor: '#6390B5', color: '#FFFFFF' }}>NEW LAUNCH — B1 Freehold</span>
    ...
    <p className="font-sans text-xl mb-2" style={{ color: 'rgba(255,255,255,0.9)' }}>21 New Industrial Road, Singapore 536213</p>
    <p className="font-sans text-base mb-6 max-w-[600px]" style={{ color: 'rgba(255,255,255,0.8)' }}>
...
function SpaceNovaLocation() {
  ...
  return (<section ref={ref} className="py-24" style={{ backgroundColor: '#fff' }}><div className="max-w-[1128px] mx-auto px-6"><div className="sn-loc grid grid-cols-1 lg:grid-cols-2 gap-12"><div>
    ...
    <p className="font-sans text-base leading-relaxed mb-8" style={{ color: '#1C1A17' }}>Space Nova is located on New Industrial Road, a well-established industrial address with excellent connectivity to MRT stations and major expressways. The location offers seamless access to the Central Business District, Changi Airport, and major industrial hubs across Singapore.</p>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {['Full-Height Glass Facade', 'Ramp-Up Access', 'Communal Sky Terrace', 'Private Attached Toilets', '4 EV Charging Lots', 'Flexible Unit Combinations', '100A Power (Select Units)', '2 Loading Bays + Heavy Vehicle'].map((a, i) => (
        <div key={i} className="flex items-center gap-3 bg-white rounded-lg p-4" style={{ border: '1px solid #E8E4DC' }}><CheckCircle size={16} color="#6390B5" /><span className="font-sans text-sm" style={{ color: '#1C1A17' }}>{a}</span></div>
      ))}
    </div>
  </div></div></div></section>)
}
...
function SharedEnquiry() {
  ...
  return (<section ref={ref} id="enquiry" style={{ backgroundColor: '#1C1A17' }} className="py-24"><div className="max-w-[1128px] mx-auto px-6"><div className="enq-content text-center">
    <p className="mono-label mb-4" style={{ color: '#F5C443' }}>REGISTER YOUR INTEREST</p>
    <h2 className="font-serif text-4xl text-white font-medium mb-4">Secure Your Unit Today</h2>
    <p className="font-sans text-base mb-8 mx-auto" style={{ color: 'rgba(255,255,255,0.8)', maxWidth: 600 }}>Register your interest for any of our new industrial and commercial launches. Priority registration gives you first-mover advantage for unit selection, pricing updates, and exclusive previews.</p>
    <div className="space-y-4 mb-6">
      <div className="flex items-center justify-center gap-3"><Phone size={18} color="#4A3E3D" /><a href="tel:+6597687722" className="font-sans text-base text-white hover:text-[#4A3E3D] transition-colors">+65 9768 7722</a></div>
      <div className="flex items-center justify-center gap-3"><Phone size={18} color="#4A3E3D" /><a href="tel:+6586882929" className="font-sans text-base text-white hover:text-[#4A3E3D] transition-colors">+65 8688 2929</a></div>
    </div>
    <div className="flex flex-wrap items-center justify-center gap-4 mt-8">
      <a href="https://wa.me/message/7EESXSAYAH2II1" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 font-sans text-sm py-3 px-6 rounded-lg text-white font-medium" style={{ backgroundColor: '#25D366' }}><MessageCircle size={16} /> WhatsApp</a>
      <a href="https://whatsapp.com/channel/0029Va0J0Mj3QxSASaTLPF0C" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 font-sans text-sm py-3 px-6 rounded-lg text-white font-medium" style={{ backgroundColor: '#25D366' }}><MessageCircle size={16} /> WhatsApp Channel</a>
      <a
        href="https://www.privyr.com/form/1U3bZG2k#sg-industrial-enquiry"
        target="_blank"
        rel="noopener noreferrer"
        className="btn-bronze inline-flex items-center gap-2 font-sans text-sm py-3 px-6 rounded-lg"
      >
...
function Disclaimer() {
  return (<section className="py-10" style={{ backgroundColor: '#F7F5F0' }}><div className="max-w-[1128px] mx-auto px-6"><div className="rounded-xl p-6" style={{ backgroundColor: '#fff', border: '1px solid #E8E4DC' }}>
    <p className="font-mono text-xs uppercase mb-2" style={{ color: '#1C1A17' }}>Disclaimer</p>
    <p className="font-sans text-xs leading-relaxed" style={{ color: '#1C1A17' }}>All information, specifications, and floor plans provided herein are subject to change without prior notice. All strata areas are subject to Strata Value Approval. The developer reserves the right to make modifications to the design, specifications, and features. Dimension measurements are subjected to final survey and are estimates only. Purchasers should verify all details with the developer's sales team before making any purchase decision. This document is a draft for agent circulation only and not for public distribution.</p>
    <p className="font-sans text-xs mt-3" style={{ color: '#1C1A17' }}><strong>Gate+ Developer:</strong> Tukang Project Pte. Ltd. | <strong>Developer Solicitor:</strong> Rajah &amp; Tann Singapore</p>
    <p className="font-sans text-xs mt-2" style={{ color: '#1C1A17' }}><strong>CT Gold:</strong> 100% fully sold. Project details subject to developer confirmation. For enquiries on similar upcoming industrial launches or resale opportunities, contact our team.</p>
    <p className="font-sans text-xs mt-2" style={{ color: '#1C1A17' }}><strong>Generations@Tannery:</strong> Floor-to-floor heights are estimates. All dimensions and specifications are subjected to final survey and approval. C&S: MKPL Architects Pte Ltd.</p>
    <p className="font-sans text-xs mt-2" style={{ color: '#1C1A17' }}><strong>SPACE@18:</strong> Developed by JVA Whampoa Pte Ltd. Information based on e-brochure. All details subject to developer confirmation.</p>
    <p className="font-sans text-xs mt-2" style={{ color: '#1C1A17' }}><strong>Space Nova:</strong> Information based on e-brochure dated 2 July 2026. All details subject to developer confirmation.</p>
    <p className="font-sans text-xs mt-2" style={{ color: '#1C1A17' }}><strong>Gourmet Xchange (GXC):</strong> Developed by CL Savour Property Pte Ltd. Information based on marketing materials dated 14 January 2026. All details subject to developer confirmation.</p>
  </div></div></section>)
}
